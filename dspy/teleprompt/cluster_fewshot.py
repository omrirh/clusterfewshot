...
class ClusterFewshot(Teleprompter):
    ...
    def _cluster_examples(self, train=True):
        ...
        self.examples2embeddings.update({
            self.get_example_hash(item): np.array(emb)
            for item, emb in zip(self.trainset if train else self.valset, examples_embeddings)
        })
        self.embeddings2examples.update({
            str(emb): item
            for emb, item in zip(examples_embeddings, self.trainset if train else self.valset)
        })
    ...
    def pick_best_fewshot_subset(self):
        ...
        if self._soft_select:
            ...
            for _lambda, fewshot_subset in self.lambda_to_fewshot.items():
                logger.info(f"\n\nTesting lambda={_lambda} sampled few-shot subset")
                ...
                if fewshot_subset_score > best_score:
                    best_lambda = _lambda
                    best_score = fewshot_subset_score

                logger.info(f"lambda={_lambda} few-shot subset scored {fewshot_subset_score:.2f}% "
                            f"on the validation set with {len(fewshot_subset)} demonstrations.")

            self.final_fewshot_subset = self.lambda_to_fewshot[best_lambda]

            logger.info(
                f"Best few-shot subset sampled recorded with lambda={best_lambda} with "
                f"({best_score}% accuracy on the validation set)")
        else:
            sampling_strategies = TASK_2_SAMPLINGS[self.task_type]
    ...
    def soft_select(
        ...
        device: str = "cpu",
        verbose: bool = True,
        tau: float = 0.05,  # entropy base strength
        ...
        Adds entropy regularization to prevent early collapse of p.
        NEW: tau_eff scaling with lambda to keep entropy competitive when lambda is large.
        """
        import math

        # Fix a canonical ordering of candidates
        candidate_hashes = list(self.ranked_examples.keys())
        M = len(candidate_hashes)

        # Build one-shot scores tensor
        one_shot_scores = torch.tensor(
            [self.ranked_examples[h] for h in candidate_hashes],
            dtype=torch.float32,
            device=device,
        )

        # Normalize to [0, 1]
        s_min = one_shot_scores.min()
        s_max = one_shot_scores.max()
        one_shot_scores = (one_shot_scores - s_min) / (s_max - s_min + 1e-8)

        # Build embedding matrix
        embs = torch.stack([
            torch.tensor(self.examples2embeddings[h], device=device, dtype=torch.float32)
            for h in candidate_hashes
        ]).to(device)

        # Cosine similarity matrix (redundancy)
        ...
        for _lambda in [0.1, 0.3, 1.0, 3.0, 10.0]:
            lambda_val = float(_lambda)  # keep as python float (fixed per sweep)

            # ✅ NEW: scale entropy strength with lambda so it doesn't get crushed
            # pick ONE scaling; (1 + λ) is the most direct.
            tau_eff = tau * (1.0 + lambda_val)
            # alternative gentler scaling:
            # tau_eff = tau * math.sqrt(1.0 + lambda_val)

            logits = torch.zeros(M, device=device, requires_grad=True)
            optimizer = torch.optim.Adam([logits], lr=lr)

            logger.info(
                f"Starting soft-select optimization with λ={lambda_val} "
                f"(tau={tau:.4f}, tau_eff={tau_eff:.4f})"
            )
            ...
            for step in range(steps):
                ...
                if verbose and step % log_step == 0:
                    logger.info(
                        ...
                        f"div={div.item():.6f} "
                        f"H={entropy.item():.4f} "
                        f"λ·div={(lambda_val * div).item():.6f} "
                        f"tau_eff·H={(tau_eff * entropy).item():.6f}"
                    )
            ...
            # Select top-N after convergence
            soft_scores = F.softmax(logits, dim=0)
            topn = torch.topk(soft_scores, N).indices.tolist()

            self.lambda_to_fewshot[_lambda] = [
                self.trainset_by_hash[candidate_hashes[i]] for i in topn
            ]

            self.visualize_soft_selection(div_lambda=lambda_val, _lambda=_lambda)

    def visualize_soft_selection(self, div_lambda, _lambda):
        """
        PCA plot of all training embeddings, highlighting selected few-shot examples.
        """
        all_examples_hashes = list(self.ranked_examples)
        embs = np.stack([
            self.examples2embeddings[ex]
            for ex in all_examples_hashes
        ])
        selected_set = {self.get_example_hash(ex) for ex in self.lambda_to_fewshot[_lambda]}

        pca = PCA(n_components=2)
        reduced = pca.fit_transform(embs)

        colors = ['red' if ex in selected_set else 'gray' for ex in all_examples_hashes]

        save_path = f"fewshot_lambda_{_lambda}.png"
        ...
        logger.info(f"Soft selection (lambda={_lambda}) PCA plot saved to {save_path}")
